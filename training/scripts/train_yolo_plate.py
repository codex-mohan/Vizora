...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLO11 Indian license plate detector.")
...
def validate_dataset(data_yaml: Path) -> dict[str, Any]:
    console.rule("[bold cyan]Plate Dataset Validation")
    ...
    if "license_plate" not in class_names:
        raise ValueError("Plate detector data.yaml must include a license_plate class")
    ...
    if image_count == 0:
        raise ValueError("No plate training/validation images found")

    table = Table(title="Plate Class Distribution")
...
def main() -> None:
    ...
    dataset_info = validate_dataset(data_yaml)

    vol_root = args.vol_root
    default_run_root = vol_root / "runs" if vol_root else Path("runs")
    run_dir = (args.run_dir or default_run_root / "plate").resolve()
    run_dir.mkdir(parents=True, exist_ok=True)
    weights_dir = (args.weights_dir or (vol_root / "weights" / "plate" if vol_root else run_dir / "exported_weights")).resolve()
    ...
    epochs = args.epochs or model_cfg.get("epochs", 80)
    ...
    console.rule("[bold magenta]Training Plate Detector")
    try:
        results = model.train(
            data=str(data_yaml),
            epochs=epochs,
